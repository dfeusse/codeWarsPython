...
the others in evenness.  program that among the given numbers finds one that is different in evenness, 
...
'''
def iq_test(string):
	mylist = map(int,string.split(' '))
	evenOrOdd = []
	for i in mylist:
		if i % 2 == 0:
			evenOrOdd.append('even')
		else:
			evenOrOdd.append('odd')
	evens = evenOrOdd.count('even')
	return evenOrOdd.index('even')+1 if evens == 1 else evenOrOdd.index('odd')+1
